...
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:@"showKeyboard" object:nil];
         [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:@"hideKeyboard" object:nil];
    }
    return self;
}
...
- (UIView *)createContainerView
{
    ...
    dialogContainer.layer.shadowColor = [UIColor blueColor].CGColor;
...
//        [closeButton.layer setCornerRadius:kCustomIOSAlertViewCornerRadius];
    ...
    UIView *linetop = [[UIView alloc] initWithFrame:CGRectMake(0, container.bounds.size.height - buttonHeight - 10,container.bounds.size.width, 1)];
    linetop.backgroundColor = [UIColor groupTableViewBackgroundColor];
    [containerView addSubview:linetop];
    
    
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(container.bounds.size.width/2, container.bounds.size.height - buttonHeight - 10, 1, buttonHeight)];
...
//    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
//                     animations:^{
                         dialogView.frame = CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - keyboardSize.height - dialogSize.height) / 2 - 50, dialogSize.width, dialogSize.height);
//                     }
//                     completion:nil
//     ];
}
...
- (void)changeFrame:(CGFloat)contentViewY{
    CGSize screenSize = [self countScreenSize];
    CGSize dialogSize = [self countDialogSize];
    dialogView.frame = CGRectMake((screenSize.width - dialogSize.width) / 2, (contentViewY - dialogSize.height), dialogSize.width, dialogSize.height);
}
