...
#import "UIControl+WGButtonExtension.h"
//最右边按钮宽度
#define WGRightButton_Width 44

//所有按钮宽度
#define WGItems_Width 46

@interface WGFontStyleBar()<UIScrollViewDelegate>
@property (nonatomic,strong) UIScrollView *scroBarView;
@property (nonatomic,strong) UIButton *autoScroBtn;
@property (nonatomic,strong) NSArray *items;

@end
@implementation WGFontStyleBar

- (UIScrollView *)scroBarView{
    if (!_scroBarView) {
        _scroBarView = [[UIScrollView alloc] init];
        _scroBarView.backgroundColor = WGCOLORA(237, 237, 237);
...
- (void)setupViews:(CGRect)frame{
    ...
    self.scroBarView.frame = CGRectMake(0,0, frame.size.width - WGRightButton_Width, KWFontBar_Height);
    
    self.autoScroBtn.frame = CGRectMake(frame.size.width - WGRightButton_Width, 0, WGRightButton_Width, KWFontBar_Height);
...
//    CGFloat itemW = (self.scroBarView.frame.size.width - KViewHeight) / itemsCount;
    for (int i = 0; i < itemsCount; i++) {
        ...
        button.frame = CGRectMake(i*WGItems_Width,0, WGItems_Width,frame.size.height);
        button.tag = i;
        [self.scroBarView addSubview:button];

    }
    self.scroBarView.contentSize = CGSizeMake(itemsCount*WGItems_Width, 0);
}
